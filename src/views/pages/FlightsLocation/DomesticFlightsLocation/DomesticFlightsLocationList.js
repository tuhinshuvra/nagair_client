...
const DomesticFlightsLocationList = () => {
    ...
    useEffect(() => {
        fetch(`${process.env.REACT_APP_NAGAIR}/api/show-domestic-flight`, {
            method: 'GET',
            headers: {
    ...
    const handleDelete = () => {
        fetch(`${process.env.REACT_APP_NAGAIR}/api/delete-domestic-flight?id=${deletingFlights}`, {
            method: "DELETE",
            headers: {
                'Content-type': 'application/json; charset=UTF-8', Authorization: `Bearer ${getCookie('token')}`,
            },
    ...
    return (
        <div>
            <h2 className="text-center  fw-bold  mt-5">Domestic Flight's Location List</h2>
            <div className=" d-flex  justify-content-end">
                <Link to="/domesticFlightAdd" className="fs-4  text-center text-decoration-none btn btn-outline-primary btn-sm   fw-bold  mt-0 mb-1  ">Add Domestic Flight Location</Link>
            </div>
...
export default DomesticFlightsLocationList;